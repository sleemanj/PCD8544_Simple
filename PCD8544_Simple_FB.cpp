...
PCD8544_Simple_FB::PCD8544_Simple_FB()
{
}

void PCD8544_Simple_FB::begin(bool invert)
{
	this->begin(invert, 0xB0, 0x04, 0x12);
}

void PCD8544_Simple_FB::begin(bool invert, uint8_t vop, uint8_t tempCoef, uint8_t bias)
{
...
size_t PCD8544_Simple_FB::write(uint8_t data)
{
...
void PCD8544_Simple_FB::clear(bool render)
{
...
uint8_t PCD8544_Simple_FB::gotoXY(uint8_t x, uint8_t y) 
{	
...
uint8_t PCD8544_Simple_FB::writeBitmap(const uint8_t *bitmap, uint8_t x, uint8_t y, uint8_t width, uint8_t height)
{
...
//void PCD8544_Simple_FB::init(void) 
//{
//	// Must reset LCD first!
//	PCD8544_PORT &= ~PIN_RESET;
//	PCD8544_PORT |= PIN_RESET;
...
void PCD8544_Simple_FB::renderAll()
{	
	this->gotoXY(0, 0);
	this->writeLcd(PCD8544_DATA, this->m_Buffer, BUF_LEN);
}

uint8_t PCD8544_Simple_FB::renderString(uint8_t x, uint8_t y, uint16_t length)
{
	if (this->gotoXY(x, y) == PCD8544_ERROR) return PCD8544_ERROR;
	length *= 6;
...
void PCD8544_Simple_FB::setPixel(uint8_t x, uint8_t y, uint8_t value)
{
	if (x >= PCD8544_X_PIXELS || y >= PCD8544_Y_PIXELS) return;
	uint8_t bank = y / 8;
...
uint8_t PCD8544_Simple_FB::writeLine(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2)
{
	if (x1 == x2 || y1 == y2)
	{
		if (y1 > y2)
...
void PCD8544_Simple_FB::swap(uint8_t &a, uint8_t &b)
{
	uint8_t temp = a;
	a = b;
	b = temp;
...
uint8_t PCD8544_Simple_FB::writeRect(uint8_t x, uint8_t y, uint8_t width, uint8_t height, bool fill)
{
...
void PCD8544_Simple_FB::writeLcd(uint8_t dataOrCommand, const uint8_t *data, uint16_t count)
{
...
void PCD8544_Simple_FB::writeLcd(uint8_t dataOrCommand, uint8_t data)
{
	PCD8544_PORT = (PCD8544_PORT & ~PINS_CE_DC) | dataOrCommand;
	SPI.transfer(data);
	PCD8544_PORT |= PIN_CE;
